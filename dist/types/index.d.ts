...
declare type FriendlyShipType = '艦' | '駆逐' | '軽巡' | '重巡' | '航巡' | '水母' | '空母' | '軽母' | '装母' | '戦艦' | '低速戦艦' | '航戦' | '高速艦' | '潜水艦' | '潜水空母' | '潜水母艦' | '海防艦';
declare type EnemyType = '敵補給艦' | '敵潜水艦' | '敵空母' | '敵軽母';
declare type OtherShipType = '他の艦';
declare type ShipType = FriendlyShipType | EnemyType | OtherShipType;
/**
 * 资源（油弹钢铝）
 */
declare type Resources = [number, number, number, number];
/**
 * 选择奖励
 */
...
declare type RewardOther = (Item | RewardChoices)[];
/**
 * 编成要求
 */
interface Group {
...
declare type ConjunctionRequirement = AndRequirement | ThenRequirement | OrRequirement;
...
interface SimpleRequirement {
     ...
     */
    category: 'simple';
    subcategory: 'equipment' | 'ship' | 'scrapequipment' | 'scrapship' | 'modernization' | 'improvement' | 'resupply' | 'repair' | 'battle';
    times: number;
    /**
     * for scrapequipment
...
declare type Result = 'A' | 'B' | 'C' | 'S' | 'クリア';
...
declare type NormalRequirement = AGouRequirement | SimpleRequirement | FleetRequirement | EquipexchangeRequirement | ExcerciseRequirement | ExpeditionRequirement | ModelconversionRequirement | ModernizationRequirement | ScrapequipmentRequirement | SinkRequirement | SortieRequirement;
declare type Requirements = ConjunctionRequirement | NormalRequirement;
/**
 * Kancolle Quest Data
 */
